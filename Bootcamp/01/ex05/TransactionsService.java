...
public class TransactionsService {
    ...
    public int getIdLastUser() throws UserNotFoundException {
        return userlist.getUserIndex(userlist.getUserCount() - 1).getId();
    }
    ...
    public String getName(int id) throws UserNotFoundException {
        try {
            return userlist.getUserId(id).getName();
        } catch (UserNotFoundException e) {
            throw new UserNotFoundException(e.getMessage());
        }
    }

    public User[] allUsers() throws UserNotFoundException {
        User[] allUser = new User[userlist.getUserCount()];
        for (int i = 0; i < userlist.getUserCount(); i++) {
            allUser[i] = userlist.getUserIndex(i);
        }
        return allUser;
    }
    ...
    public Transaction getTransaction(int idUser, UUID idTransaction) throws UserNotFoundException, TransactionNotFoundException {
        return userlist.getUserId(idUser).getTransactionsList().getTransaction(idTransaction);
    }
